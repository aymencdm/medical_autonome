    ...
    try:
        ...
        while state["running"]:
            ...
            if current_mode == RobotMode.FACE_TRACKING:
                ...
                # 1. Track Face (get calculated moves, but don't apply yet)
                nt_pan, nt_tilt = tracker.process_frame(frame, state["pan"], state["tilt"])
                
                # Retrieve capture state
                cap_data = state_machine.state_data
                patient_id = cap_data.get("patient_id")
                captured_count = cap_data.get("captured_count", 0)
                stability_counter = cap_data.get("stability_counter", 0)
                
                if tracker.locked_face_center:
                    # Face locked
                    stability_counter += 1
                else:
                    # Lost face
                ...
                # 2. Servo Control
                if stability_counter < CONFIG["TRAINING"]["STABILITY_FRAMES"]:
                    # Not stable yet, keep tracking
                    state["pan"], state["tilt"] = nt_pan, nt_tilt
                else:
                    # Stable! Stop moving servos (Freezing camera for capture)
                    pass

                # 3. Capture Logic
                if stability_counter >= CONFIG["TRAINING"]["STABILITY_FRAMES"]:
                    current_time = time.time()
                    last_capture_time = cap_data.get("last_capture_time", 0)
                    
                    if current_time - last_capture_time >= CONFIG["TRAINING"]["DELAY"]:
                        # CAPTURE NOW
                        logger.info(f"📸 Capturing frame {captured_count + 1}/{CONFIG['TRAINING']['SAMPLES']} for ID {patient_id}")
                        
                        # Save frame
                        to_save = frame.copy()
                        from pathlib import Path
                        p_dir = Path(CONFIG["FACE_RECOGNITION"]["DATASET_PATH"]) / str(patient_id)
                        p_dir.mkdir(parents=True, exist_ok=True)
                        fname = p_dir / f"capture_{captured_count}_{int(current_time)}.jpg"
                        cv2.imwrite(str(fname), to_save)
                        
                        captured_count += 1
                        cap_data["last_capture_time"] = current_time
                        cap_data["captured_count"] = captured_count
                        
                        socketio.emit('capture_progress', {"current": captured_count, "total": CONFIG["TRAINING"]["SAMPLES"]}, namespace=CONFIG["NETWORK"]["NAMESPACE"])
                        
                        if captured_count >= CONFIG["TRAINING"]["SAMPLES"]:
                            logger.info("✅ Capture session complete")
                            state_machine.transition_to(RobotMode.IDLE)
                            # Optional: Transition back to face tracking?
                            # state_machine.transition_to(RobotMode.FACE_TRACKING)

                # Update state data
                cap_data["stability_counter"] = stability_counter
...
def dispense_medicine_task(medicine_angle):
    """Task for medicine dispensing"""
    success = medicine_controller.dispense_medicine(medicine_angle)
...
@app.route('/api/patients/<int:patient_id>/capture', methods=['POST'])
def capture_faces(patient_id):
    # Trigger training capture mode
    logger.info(f"Starting capture for Patient ID {patient_id}")
    success = state_machine.transition_to(RobotMode.TRAINING_CAPTURE, {
    ...
    if success:
        ...
        return jsonify({"error": "Cannot start capture from current state"}), 400
...
def rotate_wheel():
    data = request.json
    medicine_controller.rotate_to_angle(data["angle"])
    return jsonify({"success": True})
