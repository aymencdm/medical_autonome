...
class MedicineController:
    ...
    def _angle_to_duty(self, angle):
        ...
        # For continuous rotation servos, this might need adjustment
        # For standard servos: 2.5% = 0°, 12.5% = 180°
        # For 360° servos, we use pulse width to control speed/direction
        if angle <= 180:
    ...
    async def rotate_to_angle(self, target_angle: float, speed: float = None):
    ...
    async def dispense_medicine(self, medicine_angle: float):
        """
        ...
        try:
            ...
            await self.rotate_to_angle(backward_angle)
            
            # Step 2: Rotate to medicine angle
            logger.info("Step 2/5: Rotating to medicine angle...")
            await self.rotate_to_angle(medicine_angle)
            ...
            await asyncio.sleep(CONFIG["MEDICINE"]["PICKUP_WAIT_TIME"])
    ...
    async def open_door(self):
        ...
        await asyncio.sleep(0.5)  # Wait for servo to move
